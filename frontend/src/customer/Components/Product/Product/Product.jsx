...
export default function Product() {
  ...
  useEffect(() => {
    // Only dispatch findProducts for category-based browsing, not for search
    if (!searchQuery) {
    ...
    // For search results with filters, dispatch from filter changes
    if (searchQuery && (colorValue || sizeValue || price || discount || sortValue || pageNumber || stock)) {
      const [minPrice, maxPrice] = price === null ? [0, 0] : price.split("-").map(Number);
  ...
  }, [searchQuery, param.lavelThree, colorValue, sizeValue, price, discount, sortValue, pageNumber, stock, dispatch]);

  // Loading state effect
  useEffect(() => {
    setIsLoaderOpen(customersProduct.loading || false);
                          ...
                          <Disclosure.Panel className="pt-6">
                            <FormControl>
                              ...
                              >
                                {section.options.map((option, optionIdx) => (
                                  <FormControlLabel
                                    key={`${section.id}-${optionIdx}`} // Added key prop
                                    value={option.value}
                                    control={<Radio />}
                                    label={option.label}
                                    onChange={(e) =>
                ...
                <div className="lg:col-span-4 w-full">
                  <div className="flex flex-wrap justify-center bg-white border py-5 rounded-md">
                    {customersProduct?.products?.data?.content?.map((item) => (
                      <ProductCard key={item.id} product={item} /> // Added key prop
                    ))}
                  </div>
                </div>
              </div>
